...
/// Mirrors `Core/GUI/src/lib/ollamaSettings.js::DEFAULT_OLLAMA_SETTINGS`.
/// Read via the harness pipe (`ollama.settings.read`) once the Phase 8
/// model-registry surface is fully wired; until then we mirror what
/// the Svelte side persists locally so a future cutover doesn't drop
/// a user's values.
#[derive(Debug, Clone, Default, PartialEq)]
