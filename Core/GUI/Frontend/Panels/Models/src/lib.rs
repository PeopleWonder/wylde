...
//!   * **Pull a model** — single-line TextInput + Pull button, with a
//!     row of hardware-aware recommendation chips below.  Submit kicks
//!     off `ollama.pull` streaming; the chunk loop renders a live
//!     progress bar.  Cancel drops the stream (cancel-by-disconnect).
