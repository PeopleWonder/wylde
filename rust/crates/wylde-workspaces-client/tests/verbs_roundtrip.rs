...
fn slice_0b_verbs_have_expected_policies() {
    ...
    let expected: &[(&str, TimeoutPolicy, bool, Option<u64>)] = &[
        ("workspaces.list_mru", TimeoutPolicy::Fixed(FAST), true, Some(30)),
        ("workspaces.set_active", TimeoutPolicy::Fixed(FAST), true, None),
        ("workspaces.create", TimeoutPolicy::Fixed(SLOW), false, None),
        ("workspaces.update", TimeoutPolicy::Fixed(FAST), true, None),
        ("workspaces.delete", TimeoutPolicy::Fixed(MEDIUM), false, None),
...
fn unique_service_name() -> String {
    let nanos = SystemTime::now()
        ...
        .map(|d| d.as_nanos())
        .unwrap_or(0);
    format!("wylde-workspaces-clienttest-{}-{}", std::process::id(), nanos)
}
...
async fn wrappers_send_correct_action_and_payload_and_parse_reply() {
    ...
    register_recording("workspaces.list_mru", &calls, json!({"workspaces": [], "active_id": null}));
    register_recording("workspaces.set_active", &calls, json!({"active_id": "w-1", "mru": ["w-1"]}));
    register_recording("workspaces.create", &calls, json!({"id": "w-1", "name": "P"}));
    register_recording("workspaces.update", &calls, json!({"id": "w-1", "rag_enabled": false}));
    register_recording("workspaces.delete", &calls, json!({"ok": true, "workspace_id": "w-1"}));
    register_recording("workspaces.set_persona", &calls, json!({"ok": true, "workspace_id": "w-1"}));
    register_recording("workspaces.rag_query", &calls, json!({"workspace_id": "w-1", "hits": []}));
    register_recording("workspaces.reindex", &calls, json!({"ok": true, "file_count": 0}));
    ...
    assert_eq!(client.create("/proj", Some("P")).await.unwrap()["id"], "w-1");
    assert_eq!(client.set_active("w-1").await.unwrap()["active_id"], "w-1");
    assert_eq!(client.update(json!({"workspace_id": "w-1", "rag_enabled": false})).await.unwrap()["rag_enabled"], false);
    assert_eq!(client.set_persona("w-1", "Be terse.").await.unwrap()["ok"], true);
    assert_eq!(client.rag_query("w-1", "q", Some(5)).await.unwrap()["hits"], json!([]));
    ...
    assert_eq!(payload_for("workspaces.create"), json!({"folder": "/proj", "name": "P"}));
    assert_eq!(payload_for("workspaces.set_active"), json!({"workspace_id": "w-1"}));
    assert_eq!(payload_for("workspaces.set_persona"), json!({"workspace_id": "w-1", "text": "Be terse."}));
    assert_eq!(payload_for("workspaces.rag_query"), json!({"workspace_id": "w-1", "query": "q", "k": 5}));
    assert_eq!(payload_for("workspaces.reindex"), json!({"workspace_id": "w-1"}));
    ...
    let list_calls = calls.lock().unwrap().iter().filter(|(v, _)| v == "workspaces.list_mru").count();
    assert_eq!(list_calls, 1, "list_mru should be cached after the first call");
