//! Walk-time exclusion — the one shared predicate the full RAG walk, the
//! watcher's per-file pre-filter ([`crate::stats::is_indexable_path`]), and the
//! `wylde-organize` junk-detector all consult, so a `target-dev/doc` rustdoc
//! tree (the ~58 % build-artifact pollution R4 measured) never reaches the
//! index on any path.
//!
