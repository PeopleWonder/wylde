...
| 4b | **Every GUI page loads + detects its error state, AND the panels still behave (L7)** | CI job `gui panel-walk (L7)` — headless windowed `#[gpui::test]`s across the 9 panel crates: the panel-walk mounts all 9 panels + the Workspaces subtabs under 4 backend conditions (healthy / down / error-envelope / empty), **plus** the behavioural suites (chat workspace-scoping on send, memory copy-in provenance, workspace registry nav, settings prefs dispatch, device pairing cancel), **plus** the all-surfaces chat-turn e2e (#236) — every GUI chat entry point driven composer → real turn driver → stub inference → rendered reply, with `wylde_check` rule 58 keeping the covered set complete | a page that panics, stuck-loads, or mis-detects "down" (esp. **the daemon-down case**); **and** a critical-path behavioural regression | `ci.yml` (`gui panel-walk (L7)` job) + `Core/GUI/.cargo/config.toml` (`panel-walk` alias) | ✅ **live (CI)** — windowed tests **do** run headless on the CI runner (gpui mock `TestPlatform`); **required** in both rulesets. **#56 widened it 41 → 890 enforced tests**: the alias carried `--test panel_walk`, so the required check ran `tests/panel_walk.rs` and nothing else while ~130 behavioural tests that already existed and passed were enforced by nothing. The `-p` crate scoping is load-bearing and stays (NOT `--workspace` — never link the Shell's tray-icon/wry graph or the `rust/` audio stack). ⚠️ **Do not rename this context** without adding the new one to both rulesets live first — see the note under the required-checks list. |
