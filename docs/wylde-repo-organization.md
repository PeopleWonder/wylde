...
| `Frontend/Panels/<Name>/` | `wylde-panel-<name>` | The 9 first-party panels, each a gpui `View` crate (a workspace member, enforced by `panel_crate_must_be_workspace_member`). |

The 9 first-party panel crates: `Settings` (`wylde-panel-settings`), `Workspaces` (`wylde-panel-workspaces`), `Tools` (`wylde-panel-tools`), `Memory` (`wylde-panel-memory`), `Chat` (`wylde-panel-chat`), `Models` (`wylde-panel-models`), `Dashboard` (`wylde-panel-dashboard`), `Devices` (`wylde-panel-devices`), `RemoteAccess` (`wylde-panel-remote-access`). Each panel sub-crate is typically `lib.rs` + a `<name>_panel.rs` (the `impl Render` gpui View) + `ipc.rs` (its pipe-call helpers) + a `manifest.json` registry entry. (`Panels/shared/` is a support crate, not a panel.) Two panels have been retired: `Training`, extracted 2026-06-04 (see `docs/retired-trainer-scope.md`), and `Images`, which went out-of-tree as a Service in 2026-06 and was then parked outright when ComfyUI was removed from Wylde (#234) — see [PeopleWonder/wylde-images](https://github.com/PeopleWonder/wylde-images).
