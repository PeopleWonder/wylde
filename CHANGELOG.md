...
- **Model aliases are registry-only (refs #348).** Short model names such as `coder` are part of Wylde's model registry, stored in `model_aliases.json` in the data directory. They're managed by editing that file or with the new `models.list_aliases`, `models.set_alias` and `models.remove_alias` harness verbs, take effect at runtime, and survive restarts. The OpenAI-compatible `/v1` API lists and resolves aliases from the registry on every route. An alias only counts while its model is installed, and an installed model with the same name always wins. If the harness is unreachable, aliases simply don't resolve and full model ids keep working. Wylde ships no default aliases. The sample Continue config shows how to create `coder` and `embed`.
...
- **OpenAI-compatible `/v1/chat/completions` and `/v1/completions` with fill-in-the-middle autocomplete (refs #344, part of #88).** Continue, or any OpenAI client, can now use Wylde for agent chat and tab autocomplete through one endpoint, with every request holding a VRAM lease. Chat supports text messages, tool calling in both directions (OpenAI tool calls in, Ollama tool calls translated back out), sampling options, `max_tokens`, `stop`, `seed`, and `response_format` (JSON mode or a JSON schema), streamed or not, with token usage.
  ...
  - **Latest request wins:** only the newest autocomplete request per device keeps running, so fast typing never queues work in front of the agent. Cancelled requests keep the model loaded.
- **OpenAI-compatible `/v1` API on the gateway: models and embeddings (refs #343, part of #88).** The second of four steps toward one OpenAI endpoint that Continue and any other OpenAI client can use. Clients authenticate with a Wylde device token as their API key (`Authorization: Bearer <token>`). Any verified device can call `/v1`, since the gateway never runs tools on these routes. Errors come back in OpenAI's `{error: {message, type, param, code}}` shape. `/v1` has its own per-device rate limit (`WYLDE_RATE_LIMIT_OPENAI_PER_MIN`, default 600) so autocomplete traffic can't use up the 60/min budget of Wylde's own chat. `GET /v1/models` lists the chat models the GUI shows plus the installed embedding models, falling back to everything Ollama has installed if the harness is down. Short model aliases (for example `coder`), listed alongside real ids, let a client config survive a model swap (they live in the model registry, see #348). `POST /v1/embeddings` runs through `wylde-ollama`, so it holds a VRAM lease. Chat and completions (including autocomplete) follow in #344.
- **`wylde-ollama` backend for the OpenAI-compatible API: generate actions, FIM admission, and reload-free requests (refs #342, part of #88).** The first of four steps toward one OpenAI endpoint on the gateway that Continue and any other OpenAI client can use for chat, autocomplete and embeddings, with VRAM brokered. New `ollama.generate` and `ollama.generate_stream` actions put `POST /api/generate` (completions and fill-in-the-middle) behind a VRAM lease that every exit path releases, passing `prompt`, `suffix`, `raw` and `options` straight through. A `fim: true` call is autocomplete. It gets a higher lease priority (`WYLDE_OLLAMA_FIM_PRIORITY`, default 70) but never makes an agent's model give up VRAM: if the autocomplete model isn't already loaded and wouldn't fit in free VRAM, it gets a fast `insufficient_vram` (no suggestion) instead of forcing an eviction. Leases from this service never preempt. Two opt-in per-call knobs fix problems measured on the rig. `pin_load_options: true` pins `num_ctx` to the loaded model's context, because Ollama reloads a model whenever a request's context differs, even when the request just omits it (~2 s each time). `evict_on_cancel: false` stops a client disconnect from unloading the model, which autocomplete needs since it cancels on nearly every keystroke. Every existing caller keeps today's behaviour. The knobs and the admission rule are documented in the module docs of `rust/crates/wylde-ollama/src/actions/generate.rs`, `admission.rs`, `stream_relay.rs` and `src/load_opts.rs`.
- **Wylde-managed embedded n8n (refs #338).** Wylde now runs a local n8n end to end, instead of fronting an external instance the user had to install, start and configure. The lifecycle daemon launches the engine as a new core service, `wylde-n8n-engine` (a Node runtime, so no Wylde image; its health is a probe of the n8n port). `wylde-n8n` attaches to it and fronts it over the pipe. n8n binds loopback-only at `127.0.0.1:5678`, and its editor is embedded in the Workflows panel. That panel is a `wry`-free gpui view: it shows engine status, Reload and Open in browser, and asks the Shell to host the editor over its content region through a new `embed_bus` seam in `wylde-gui-pipe`. The Shell reuses the WebView host and shared-auth bootstrap it already runs for iframe panels. Lifecycle mints and owns the n8n identity, and `wylde-n8n` provisions the owner account from it, so there is no separate sign-in. Workflows persist in a durable out-of-tree data dir whose stable encryption key keeps saved credentials usable across restarts. Randomness for that identity goes through `wylde_shared::rng` (#290). Set `WYLDE_N8N_MANAGED=0` to keep using a user-managed n8n.
- **Bi-temporal memory graph, behind a default-off toggle (refs #40).** With `WYLDE_TEMPORAL_MEMORY=1`, typed memgraph edges carry valid time and transaction time (`valid_from`/`valid_to`, `tx_from`/`tx_to`). A re-relate supersedes the old edge instead of overwriting it, an unrelate closes the validity window instead of deleting, and a correction retires the old belief without losing history. `traverse` gains an optional `as_of`, so a query can read the graph as it was, or as it was believed to be, at a past moment. With the toggle off, relational behaviour is byte-identical, and a live test pins that. The model lives in `memgraph/temporal/`, with its DB-free unit tests in `temporal/tests.rs` (rule 20). The seven live-graph tests serialize on a `DB_LOCK` and run in CI's live-graph leg.
...
- **MCP `resources/list` and workspace `resources/read` work again (#357).** They asked the harness for the workspace list, but the harness stopped serving `workspaces.*` when that moved to the `wylde-workspaces` service, so both calls always failed. They now use the workspaces service (override: `WYLDE_GATEWAY_WORKSPACES_SERVICE`). `resources/list` fails only when neither conversations nor workspaces can be listed. The harness also reads one env var for the workspaces service, `WYLDE_HARNESS_WORKSPACES_SERVICE`; chat export used `WYLDE_WORKSPACES_SERVICE`, which still works as an alias.

- **A down Neo4j no longer stalls concept builds for ~50 s (#355).** neo4rs connects lazily, so the workspace graph client's `connect_timeout` never applied. Against a down server, the first query retried "connection refused" with backoff for about 50 s before the fail-soft concept projection or ingest `upsert` gave up. The client now probes the server with `RETURN 1`, bounded by `WYLDE_BOLT_CONNECT_TIMEOUT_SECS` (default 5 s), before it caches the connection pool. The `wylde-workspaces` unit tests now pin the graph URL to a dead port, so they no longer wait on the connect (about 10 minutes of every CI run) or write into a running rig's real graph.

- **CI is green again under rustc 1.98 and the September RustSec advisories (closes #335).** develop turned red without any code change. RUSTSEC-2026-0285 (rustls: TLS 1.3 handshake messages accepted across encryption-level boundaries) and RUSTSEC-2026-0258 (h2: unbounded empty DATA frames) failed `cargo-deny`. rustc 1.98's clippy also flagged `result_large_err` on three `wylde-gateway` helpers that return an axum `Response` as their error, and `manual_slice_fill` in the graph physics step. The fix bumps rustls to 0.23.45 and h2 to 0.4.19 across the lockfiles. The gateway helpers now box the error `Response`, and callers dereference it on early return; behaviour is unchanged and all 302 gateway tests pass. The physics force buffers now reset with `slice::fill`.
