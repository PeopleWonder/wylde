...
- **Wylde-managed embedded n8n (refs #338).** `wylde-n8n` now owns a local n8n end to end, instead of fronting an external instance the user had to install, start and configure. n8n binds loopback-only at `127.0.0.1:5678` with its editor embedded in a panel. Wylde provisions and owns the n8n owner identity, so there is no separate sign-in, and workflows persist in a durable out-of-tree data dir whose stable encryption key keeps saved credentials usable across restarts. Randomness for that identity goes through `wylde_shared::rng` (#290).
