...
- **Model aliases live in the model registry (refs #348).** Short model names such as `coder` are now part of Wylde's model registry, stored in `model_aliases.json` in the data directory. They're managed with the new `models.list_aliases`, `models.set_alias` and `models.remove_alias` harness verbs, take effect at runtime, and survive restarts. The OpenAI-compatible `/v1` API lists and resolves aliases from the registry on every route. An alias only counts while its model is installed, and an installed model with the same name always wins. `WYLDE_OPENAI_MODEL_ALIASES` is deprecated: it still works, but only for names the registry doesn't define, and Wylde logs a warning when it's set.
